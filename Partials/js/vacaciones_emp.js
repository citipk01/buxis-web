...
function CambioFecIni(campo){
	...
	if(fila.ControlByName('fec_fin').Value() != '' && campo.Value() != ''){
		if(GetDateFromStr(campo.Value()) > GetDateFromStr(fila.ControlByName('fec_fin').Value())){
			...
			fila.ControlByName('dias').Value(DiffDatesStr(campo.Value(), fila.ControlByName('fec_fin').Value()) +1 );
		}
	}
	if(campo.Value() != ''){
...
function CambioDias(campo){
	...
	if(fila.ControlByName('fec_ini').Value() != '' && campo.Value() != ''){
		fila.ControlByName('fec_fin').Value(AddDaysToDateStr(fila.ControlByName('fec_ini').Value(), parseInt(campo.Value()) - 1),true);
	}
...
function CambioFecFin(campo){
	...
	if(fila.ControlByName('fec_ini').Value() != '' && campo.Value() != ''){
		if(GetDateFromStr(campo.Value()) < GetDateFromStr(fila.ControlByName('fec_ini').Value())){
			...
			fila.ControlByName('dias').Value(DiffDatesStr(fila.ControlByName('fec_ini').Value(), campo.Value()) + 1 ); 
		}
	}
...
function CambioRestantes(campo){
	if(campo.Value() < 0){
		...
		if (campo.Value() > 0){
			...
			// 2019-09-27: La cantidad de fragmenntación indica cuántas veces se pueden dividir las vacaciones - if (cantLineas >= parseInt(bxF('max_frag').Value())){
