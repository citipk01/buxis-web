...
<%
                ...
                try {
                               var xmldoc4 = Server.CreateObject("ADODB.Stream")
                               Response.Write('<H1>ADODB.Stream Test OK</H1>')
                } catch(e) {
                               Response.Write('<H1>4 '+e.description+'</H1>')
                }

                try {
                               var xmldoc5 = Server.CreateObject("ADODB.Connection")
                               Response.Write('<H1>ADODB.Connection Test OK</H1>')
                } catch(e) {
                               Response.Write('<H1>5 '+e.description+'</H1>')
                }
