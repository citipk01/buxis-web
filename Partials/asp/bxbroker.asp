...
<%
	...
	if (doLog){
		...
		file.WriteLine("");
		mensaje = "********** COMIENZO [" +getCurrentDateAsString()+ "] **********";
		//file.WriteLine("********** COMIENZO **********");
	...
	if (srvType.toUpperCase() == 'SESSION') {
		...
		var jsonOutput = "";
		var TKN = Request.Cookies('buxisseg')('token');
			...
			default:
				...
				var auxError = true
					...
					if (doLog){
						fso = new ActiveXObject("Scripting.FileSystemObject");
						file = fso.OpenTextFile(rootdirectory + '/logs/bxbroker.log',8,true);
						file.WriteLine("TKN CHECK ID: " + srvId);
						file.WriteLine("TKN CHECK TKN: " + TKN);
						file.close();
					}
				...
				if (!auxError) {
					...
					if (data.status == 0) {
						jsonOutput = '{"type": [{"name": "ERROR","type": "char","length": 50,"scale": 0}],"data": [{"ERROR": "Debe iniciar sesión."}]}'  + TKN 
